...
#import "CreditDetailController.h"

#define kPlistnameWhole @"/CreditList.plist"
#define kPlistnamePart @"CreditList"
#define kNibName @"CreditDetailView"

@implementation CreditViewController

@synthesize allCredit;
@synthesize totalCredit;

- (IBAction) add: (id) sender
{
	if (childController == nil)
		childController = [[CreditDetailController alloc] 
						   initWithNibName:@"CreditDetailView" bundle:nil];
	childController.all = self.allCredit;
	
	childController.title = @"Add C.C. Debt";
...
{	
	...
	if([[NSFileManager defaultManager] fileExistsAtPath:documentsPath] == NO)
	{
		...
		self.allCredit = temp;
		[allCredit writeToFile:documentsPath atomically:YES];
		[temp release];
	}
	[super viewDidLoad];
}
...
- (void) viewWillAppear: (BOOL)animated
{
	...
	self.allCredit = tempDictionary;
	childController.all = self.allCredit;
	self.allKeys = [allCredit keysSortedByValueUsingSelector:@selector(compare:)];
	...
	{
		NSString *tempIncome = [allCredit objectForKey:key];
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	cell.price.text = [allCredit objectForKey:cell.name.text];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath 
{
	...
	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Attention" 
													message:@"Please use the detail disclosure button on the right to edit this credit card" 
												   delegate:nil 
...
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath
{
	...
	childController.price = [[allCredit objectForKey:[allKeys objectAtIndex:row]] substringFromIndex:1];
